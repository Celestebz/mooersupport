...
def get_pdf_path(product_model, pdf_dir=r"e:\My Docment\Celeste\客服\MOOER产品说明书"):
    ...
    product_pdf_map = {
        ...
        "Prime P1": "P1_Manual_EN.pdf",
        "Prime P2": "P2_Manual_EN.pdf",
        "P1": "P1_Manual_EN.pdf",
        "P2": "P2_Manual_EN.pdf",
        ...
        "GE100 Pro": "GE100 Pro & Pro li_Manual_EN_V01_2026.04.24.pdf",
        "GE100 Pro Li": "GE100 Pro & Pro li_Manual_EN_V01_2026.04.24.pdf",
        "GE100 Pro li": "GE100 Pro & Pro li_Manual_EN_V01_2026.04.24.pdf",
...
def _get_cache_path(pdf_path, cache_dir=CACHE_DIR):
    """根据PDF文件路径，生成对应的缓存txt路径"""
...
def _actual_extraction_logic(file_path):
    ...
    if os.path.exists(cache_path):
        try:
            with open(cache_path, "r", encoding="utf-8") as f:
                cached = f.read()
            if cached.strip():
                return _detect_and_fix_caesar_cipher(cached)
        except Exception:
            pass  # 缓存损坏则回退到PDF提取

    text = ""
    
    # 1. 尝试使用 PyMuPDF (fitz) - 速度最快且最稳定
    try:
        import fitz
        ...
        if t and t.strip() and "(cid:" not in t[:500] and len(set(t[:500])) > 10:
            return _detect_and_fix_caesar_cipher(t)
    except ImportError:
        pass
    ...
    try:
        ...
        if t and t.strip():
            return _detect_and_fix_caesar_cipher(
                ''.join(char for char in t if char.isprintable() or char in '\n\t\r\f')
            )
    except Exception as e:
...
def _detect_and_fix_caesar_cipher(text, sample_size=1000, threshold=0.30, offset=0x1F):
    """
    检测文本是否为 Caesar cipher 乱码，并尝试自动修复。

    某些 PDF（如 Prime P1/P2）使用非标准字体编码，每个可打印字符
    的字节值被人为偏移了 +0x1F。例如 'O' (0x4F) 被编码为 '0' (0x30)。
    这会导致所有 PDF 文本提取库返回乱码。

    检测方法：统计文本中低位字节（0x01-0x60）的占比。正常英文文本
    的字符主要集中在 0x20-0x7E 范围，不会有大量 <0x20 的字符。
    如果低位字节占比超过阈值，则判定为 Caesar cipher 乱码并解码。

    防二次解码：先用简单英文词频检测，如果文本已有大量常见英文单词，
    说明已经解码过，直接返回原文本。
    """
    ...
    common_words = ['the', 'and', 'for', 'with', 'this', 'that', 'your', 'from', 'have',
                    'will', 'when', 'mode', 'power', 'audio', 'input', 'output', 'preset',
                    'button', 'effect', 'volume', 'please', 'reset', 'tone', 'menu', 'page']
    word_count = sum(1 for w in common_words if f' {w} ' in text_lower or text_lower.startswith(w + ' '))
    if word_count >= 3:
        return text  # 已经解码过的正常英文文本
    ...
    # 统计字节值在 0x01-0x60 范围的字符数（排除标准控制字符）
    low_byte_count = sum(
        1 for c in sample
        if ord(c) not in (0x09, 0x0A, 0x0D)  # 排除 tab, LF, CR
        and 0x01 <= ord(c) <= 0x60
    )

    ratio = low_byte_count / total
    if ratio < threshold:
        return text  # 正常文本，不需要修复

    # 检测到 Caesar cipher，执行解码
    # 对 0x01-0x60 范围的字符加上偏移量
    # 排除：控制字符 (0x09 TAB, 0x0A LF, 0x0D CR) 和 0x3F '?'（PDF 提取库的"无法解码"占位符）
    decoded = []
    for c in text:
        code = ord(c)
        if 0x01 <= code <= 0x60 and code not in (0x09, 0x0A, 0x0D, 0x3F):
            decoded.append(chr(code + offset))
        else:
            decoded.append(c)

    fixed = ''.join(decoded)
    # 只保留可打印字符和换行
    fixed = ''.join(ch for ch in fixed if ch.isprintable() or ch in '\n\r\t')
